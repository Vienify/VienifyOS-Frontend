...
import { Api, Card, Mkt, Notify, inpCls } from "./shared";

type Doc = Mkt["documents"][number];
...
export default function DocsTab({ mk, isLeader, api, notify, reload, askConfirm }: {
  mk: Mkt; isLeader: boolean; api: Api; notify: Notify; reload: () => void;
  ...
  async function uploadDoc(file: File) {
    ...
    const r = await api("/api/marketing/documents", { method: "POST", body: JSON.stringify({ name: file.name, mime: file.type, data: b64 }) });
  ...
  async function openDoc(d: { id: number; name: string }, download: boolean) {
    const r = await api(`/api/marketing/documents/${d.id}${download ? "?download=1" : ""}`);
  ...
  async function openPreview(d: Doc) {
    if (!d.hasFile) return;
    const r = await api(`/api/marketing/documents/${d.id}`);
    if (!r.ok) return notify((await r.json()).message || "Không mở được file");
  ...
  async function deleteDoc(d: Doc) {
    const r = await api(`/api/marketing/documents/${d.id}`, { method: "DELETE" });
    if (!r.ok) return notify((await r.json()).message || "Lỗi");
  ...
  async function submitRename() {
    ...
    const r = await api(`/api/marketing/documents/${renameDoc.id}`, { method: "PUT", body: JSON.stringify({ name: renameDoc.name.trim() }) });
    if (!r.ok) return notify((await r.json()).message || "Lỗi");
  ...
  const docs = mk.documents.filter((d) =>
  ...
  return (
    <Card title={`Tài liệu (${mk.documents.length})`}>
