...
import { Api, Badge, Biz, Card, DOW, Me, Meeting, Notify, inpCls, ymd } from "./shared";

export default function MeetingsTab({ biz, members, me, isLeader, dept, api, notify, reload }: {
  biz: Biz; members: Profile[]; me: Me; isLeader: boolean; dept: string; api: Api; notify: Notify; reload: () => void;
}) {
  ...
  for (const m of biz.meetings) { const d = (m.time || "").slice(0, 10); if (d) (byDay[d] ||= []).push(m); }
  ...
  return (
    <Card title={`Lịch họp (${biz.meetings.length})`}>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input className={inpCls + " w-full pl-9"} placeholder="VD: Họp doanh số tuần" value={meetForm.title}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(DEPARTMENTS).filter(([k]) => k !== dept).map(([k, l]) => (
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-neutral-800/80">
                ...
                <button onClick={async () => {
                  ...
                  const r = await api(meetForm.id ? `/api/business/meetings/${meetForm.id}` : "/api/business/meetings",
                    ...
                    {(isLeader || (m.createdBy === me?.id && m.status === "Chờ duyệt")) && (
                      <div className="flex gap-2 pt-1">
                        {isLeader && m.status === "Chờ duyệt" && (
                          <>
                            <button onClick={async () => { await api(`/api/business/meetings/${m.id}`, { method: "PUT", body: JSON.stringify({ action: "approve" }) }); notify("Đã duyệt lịch họp", "success"); reload(); }}
                              className="px-3 py-1 rounded-lg bg-white text-black hover:bg-neutral-200 text-xs font-semibold">Duyệt</button>
                            <button onClick={async () => { await api(`/api/business/meetings/${m.id}`, { method: "PUT", body: JSON.stringify({ action: "reject" }) }); notify("Đã từ chối lịch họp", "success"); reload(); }}
