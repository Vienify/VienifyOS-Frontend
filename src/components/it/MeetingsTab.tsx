...
import { Api, Badge, Card, DOW, Me, Meeting, It, Notify, inpCls, ymd } from "./shared";

export default function MeetingsTab({ it, members, me, isLeader, api, notify, reload }: {
  it: It; members: Profile[]; me: Me; isLeader: boolean; api: Api; notify: Notify; reload: () => void;
}) {
  ...
  for (const m of it.meetings) { const d = (m.time || "").slice(0, 10); if (d) (byDay[d] ||= []).push(m); }
  ...
  return (
    <Card title={`Lịch họp (${it.meetings.length})`}>
          ...
          <div className="bg-black border border-neutral-800 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-2xl"
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(DEPARTMENTS).filter(([k]) => k !== "it").map(([k, l]) => (
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-neutral-800/80">
                ...
                <button onClick={async () => {
                  ...
                  const r = await api(meetForm.id ? `/api/it/meetings/${meetForm.id}` : "/api/it/meetings",
                    ...
                    {(isLeader || (m.createdBy === me?.id && m.status === "Chờ duyệt")) && (
                      <div className="flex gap-2 pt-1">
                        {isLeader && m.status === "Chờ duyệt" && (
                          <>
                            <button onClick={async () => { await api(`/api/it/meetings/${m.id}`, { method: "PUT", body: JSON.stringify({ action: "approve" }) }); notify("Đã duyệt lịch họp", "success"); reload(); }}
                              className="px-3 py-1 rounded-lg bg-white text-black hover:bg-neutral-200 text-xs font-semibold">Duyệt</button>
                            <button onClick={async () => { await api(`/api/it/meetings/${m.id}`, { method: "PUT", body: JSON.stringify({ action: "reject" }) }); notify("Đã từ chối lịch họp", "success"); reload(); }}
