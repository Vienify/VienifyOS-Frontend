...
import { Api, Card, It, Notify, inpCls } from "./shared";

type Doc = It["documents"][number];
...
export default function DocsTab({ it, isLeader, api, notify, reload, askConfirm }: {
  it: It; isLeader: boolean; api: Api; notify: Notify; reload: () => void;
  ...
  async function uploadDoc(file: File) {
    ...
    const r = await api("/api/it/documents", { method: "POST", body: JSON.stringify({ name: file.name, mime: file.type, data: b64 }) });
  ...
  async function openDoc(d: { id: number; name: string }, download: boolean) {
    const r = await api(`/api/it/documents/${d.id}${download ? "?download=1" : ""}`);
  ...
  async function openPreview(d: Doc) {
    if (!d.hasFile) return;
    const r = await api(`/api/it/documents/${d.id}`);
    if (!r.ok) return notify((await r.json()).message || "Không mở được file");
  ...
  async function deleteDoc(d: Doc) {
    const r = await api(`/api/it/documents/${d.id}`, { method: "DELETE" });
    if (!r.ok) return notify((await r.json()).message || "Lỗi");
  ...
  async function submitRename() {
    ...
    const r = await api(`/api/it/documents/${renameDoc.id}`, { method: "PUT", body: JSON.stringify({ name: renameDoc.name.trim() }) });
    if (!r.ok) return notify((await r.json()).message || "Lỗi");
  ...
  const docs = it.documents.filter((d) =>
  ...
  return (
    <Card title={`Tài liệu (${it.documents.length})`}>
