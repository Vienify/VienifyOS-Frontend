"use client";
// Workspace phòng Nhân sự — shell chính: header, sidebar, điều phối tab và modal chung
// Nội dung từng tab nằm ở src/components/hr/*
import { useEffect, useState } from "react";
import Link from "next/link";
import { API, DEPARTMENTS, Profile, decodeToken, getToken } from "@/lib/auth";
import NotificationBell from "@/components/NotificationBell";
import UserChip from "@/components/UserChip";
import { Hr, MENU } from "@/components/hr/shared";
import HomeTab from "@/components/hr/HomeTab";
import LeavesTab from "@/components/hr/LeavesTab";
import RecruitTab from "@/components/hr/RecruitTab";
import EmployeesTab from "@/components/hr/EmployeesTab";
import MeetingsTab from "@/components/hr/MeetingsTab";
import DocsTab from "@/components/hr/DocsTab";
import MembersTab from "@/components/hr/MembersTab";
...
export default function HRWorkspace() {
  ...
  const reload = () => {
    api("/api/hr/data").then((r) => r.json()).then(setHr);
    api("/api/departments/hr").then((r) => r.json()).then((d) => setMembers(d.members || []));
  ...
  if (!hr) return <main className="min-h-screen bg-black text-slate-400 grid place-items-center">Đang tải...</main>;
    ...
    <main className="min-h-screen bg-black text-slate-100 flex flex-col">
      <header className="sticky top-0 z-10 border-b border-slate-800/80 bg-black/80 backdrop-blur">
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold tracking-tight">{DEPARTMENTS["hr"]} · Khu vực quản lý</h1>
          </div>
          <div className="flex items-center gap-4 text-sm">
            <Link href="/hr" className="px-3 py-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 transition">Trang chủ phòng</Link>
            <NotificationBell />
            <UserChip />
          </div>
        </div>
      ...
      <div className="flex flex-1">
        <aside className="w-64 shrink-0 border-r border-slate-800/80 p-3 space-y-1 sticky top-[61px] h-[calc(100vh-61px)]">
          ...
          {MENU.map((m) => (
            <button key={m.id} onClick={() => setTab(m.id)}
              ...
              {m.label}
              {m.id === "leaves" && pendingLeaves.length > 0 && (
                <span className="ml-auto text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-red-500 text-white">{pendingLeaves.length}</span>
              )}
            </button>
          ))}
        </aside>
        ...
        <div className="flex-1 p-6 space-y-12 min-w-0">
          {tab === "home" && <HomeTab hr={hr} setTab={setTab} />}
          {tab === "leaves" && <LeavesTab hr={hr} put={put} />}
          {tab === "recruit" && <RecruitTab hr={hr} notify={notify} put={put} post={post} />}
          {tab === "employees" && <EmployeesTab hr={hr} isLeader={isLeader} put={put} />}
          {tab === "meetings" && (
            <MeetingsTab hr={hr} members={members} me={me} isLeader={isLeader} api={api} notify={notify} reload={reload} />
          )}
          {tab === "docs" && (
            <DocsTab hr={hr} isLeader={isLeader} api={api} notify={notify} reload={reload}
